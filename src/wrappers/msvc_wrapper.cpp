...
bool msvc_wrapper_t::filter_cache_hit(const cache_entry_t& entry) {
  for (auto& dependency : entry.dependency_records()) {
    ...
    const auto it = m_dependencies.find(include);
    const auto digest = (it != m_dependencies.end()) ? it->second : [&]() {
      // Calculate and record the current digest of the file, in case another source file depends on
      // the same file.
      hasher_t hasher;
      hasher.update_from_file(include);
      const auto digest = hasher.final();
      m_dependencies[include] = digest;
      return digest;
    }();
  ...
  for (auto& miss_info : miss_infos) {
    ...
    for (const auto& include : src_deps) {
      ...
      if (it != m_dependencies.end()) {
        dependencies[include] = it->second;
        continue;
      }
